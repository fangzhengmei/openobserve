...
**查询链路的两个独立分支**（必须先理解这个前提）：
- ✅ **本地查询**：直接扫描 `data_wal_dir/files/` 目录，**不依赖 `file_list` 元数据表**
- ✅ **对象存储查询**：通过 `file_list::query()` 从元数据表获取文件，**只有写入 `file_list` 的文件才可见**

| 判定维度 | 重放未完成 | 上传等待 | 上传进行中 | 上传完成待清理 |
|---------|-----------|---------|-----------|-------------|
| **logs/ 有 .wal?** | ✅ 是 | ❌ 否 | ❌ 否 | ❌ 否 |
| **files/ 有 .parquet?** | ❌ 无 | ✅ 是 | ✅ 是 | ⚠️ 有（待删） |
| **对象存储物理有文件?** | ❌ 否 | ❌ 否 | ✅ 是（物理存在） | ✅ 是 |
| **file_list 有元数据?** | ❌ 否 | ❌ 否 | ❌ 否（未写入） | ✅ 是 |
| **本地可查询?** | ❌ 否 | ✅ 是 | ✅ 是 | ✅ 是 |
| **对象存储可查询?** | ❌ 否 | ❌ 否 | ❌ 否（元数据未写） | ✅ 是 |

> **关键修正**：之前"上传中时存储部分可查"是错误的。即使文件已上传到对象存储，只要 `file_list` 元数据未写入，**对象存储查询完全不可见**。但本地查询仍然可见（因为本地文件还在）。
>
...
3. **查询特征**
   - 本地可查（文件还在，本地查询直接扫描 `files/` 目录，不依赖 `file_list`）
   - 对象存储**完全不可查**（即使 `storage::put` 已完成，只要 `db::file_list::set` 未写入，查询时通过 `file_list::query()` 就看不到这个文件）
...
| 对象存储物理有文件，但**存储查询**查不到 | file_list 元数据未写入 | 检查 `db/file_list` 表是否有该文件记录 |
| **本地查询**可查但**存储查询**不可查 | 文件已上传但元数据未写，或本地文件未删 | 检查 file_list 表 + 检查 files/ 目录 |
...
| storage::put 日志显示成功，但查询不到 | db::file_list::set 失败 | 搜索日志中 `db::file_list::set` 或 `move files to s3` 相关错误 |
...
| 本地查询 file_list 获取 | `src/service/search/grpc/wal.rs` | 484 |
| 存储查询 file_list 获取 | `src/service/promql/search/grpc/storage.rs` | 263 |
| file_list 查询接口 | `src/infra/src/file_list/mod.rs` | 295 |
...
- **上传三步顺序修正**：真实顺序是**对象存储上传 → 元数据写入 → 旧本地文件清理**，而非先删本地后上传
- **查询可见性修正**：查询分两个独立分支，本地查询直接扫描 `files/` 目录（不依赖 file_list），对象存储查询通过 `file_list::query()` 选文件（必须有元数据才可见）。即使文件已上传到对象存储，只要 `file_list` 未写入，**对象存储查询完全不可见**，不存在"部分可查"。
