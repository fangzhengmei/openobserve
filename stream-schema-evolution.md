...
`handle_diff_schema` 中 (`service/schema.rs:353-437`) 自动启用 UDS：

1. `allow_user_defined_schemas` 配置开启
2. 日志类型流
3. UDS 尚未启用
4. schema 字段数超过 `schema_max_fields_to_enable_uds` 阈值

### 5.2 UDS 字段选择策略

自动启用时按优先级选择字段：
1. 保留字段：`_timestamp`, `_id`, `_original`, `_all`
2. 默认 FTS 字段：`SQL_FULL_TEXT_SEARCH_FIELDS`
3. 默认二级索引字段：`SQL_SECONDARY_INDEX_SEARCH_FIELDS`
4. 用户配置的 FTS/索引字段
5. 当前 schema 中的其他字段（直到达到阈值）
...
4. **高并发设计**：本地锁 + 双重检查 + 数据库事务重试
5. **性能优化**：内存缓存 + 哈希快速比较 + 快/慢路径分离
6. **持久化历史**：独立的 schema_history 表记录每次演进
7. **灵活配置**：支持用户定义 schema（UDS）限制字段爆炸
