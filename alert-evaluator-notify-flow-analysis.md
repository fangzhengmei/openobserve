...
分组发送失败时的处理逻辑与去重/事件关联的降级策略完全不同，采用 **"入批即确认、失败即丢弃"** 的设计原则。

**同步发送失败处理** (`handlers.rs:923-937`)：
```rust
if batch_ready {
    if let Some(batch) = get_ready_batch(&fingerprint)
        && let Err(e) = send_grouped_notification_sync(batch).await
    {
        log::error!(...);  // 🔴 只记录错误，不做任何回退！
    }
}
// 无论发送成功失败，继续执行后面的触发器更新逻辑...
```

**统一退出逻辑** (`handlers.rs:945-963`)：
```rust
// 无论分组通知是否发送成功，都会执行以下操作：
trigger_data_stream.grouped = Some(true);       // 标记为已分组
new_trigger.data = json::to_string(&trigger_data).unwrap();
db::scheduler::update_trigger(new_trigger, ...).await?;  // 更新触发器到数据库
publish_triggers_usage(trigger_data_stream);
return Ok(());  // 🟡 正常返回，不进入通知重试流程
```

**后台超时发送失败处理** (`alert_grouping.rs:45-52`)：
```rust
for batch in get_expired_batches() {
    if let Err(e) = send_grouped_notification(batch).await {
        log::error!(...);  // 🔴 只记录错误，批次已从内存移除
    }
}
```
...
1. **语义不可逆性**：分组是一个**契约性操作**。用户开启分组即表示期望"这类告警以批量方式通知"，如果分组失败就回退到直发，会违反用户预期——用户可能为分组通知配置了专门的处理流程（如聚合降噪、工单系统对接），直发会导致下游系统行为混乱。

2. **数据已不可逆**：
   - `get_ready_batch()` 内部调用 `PENDING_BATCHES.remove()`，批次一旦取出就从内存中**永久删除**
   - 批次中的告警可能来自多个不同的触发器执行，无法追溯并逐个回退
   - 即使想回退，也没有保存每个告警的原始触发器上下文（如 trace_id、触发时间等）

3. **性能与一致性权衡**：分组的核心价值是减少通知数量，如果失败就逐个直发，在极端情况下（如通知通道短暂故障）反而会产生**告警风暴**，违背分组设计的初衷。

**为什么不会进入通知重试流程？**

1. **重试上下文已丢失**：直接通知的重试机制依赖于当前触发器的 `retries` 计数和状态更新。但分组发送是在**触发器处理流程的末尾**调用的，此时触发器已经准备好更新为 Completed 状态。如果要重试分组通知，需要：
   - 单独的分组重试队列（当前未实现）
   - 保存整个批次的完整数据（内存开销大）
   - 处理批次中部分告警已过期的边界情况

2. **分组通知是"尽力而为"**：设计上分组通知的优先级低于单次告警通知。分组通知丢失的代价是"用户没有收到聚合通知"，但告警本身已经被系统记录（通过 `update_trigger` 更新了触发器状态），用户仍可以通过告警历史页面查到。

3. **监控告警兜底**：系统通过 `ALERT_GROUPING_SEND_ERRORS_TOTAL` 指标暴露分组发送失败，运维人员可以配置监控来发现并处理这类问题，而非让系统自动重试。

**失败场景下的系统状态**：

| 组件 | 状态 | 影响 |
|------|------|------|
| 触发器 | 更新为 Completed，next_run_at 已推进 | 下次执行不受影响，告警评估继续正常进行 |
| 批次内存 | 已从 PENDING_BATCHES 移除 | 无法重试，数据丢失 |
| 告警历史 | 已记录 grouped=true | 用户在UI上能看到该告警已"分组处理" |
| 指标 | ALERT_GROUPING_SEND_ERRORS_TOTAL 递增 | 可通过监控发现异常 |

**这种设计的核心假设**：分组通知失败是**偶发且可观测**的，通过指标监控+人工介入的成本，低于实现可靠的分组重试机制的工程成本。
...
| 模块 | 处理视角 | 失败处理 | 影响范围 |
|------|---------|---------|---------|
| 分组（grouping）| 首行驱动 | 批次发送失败仅记录日志 | 整批告警可能丢失分组通知，但不影响后续告警触发 |
| 去重（deduplication）| 逐行处理 | 返回原始数据，继续流程 | 可能收到重复告警，但不会漏报；单行异常不影响其他行 |
| 事件关联（incidents）| 首行驱动 | 标记为未处理，执行直接通知 | 不关联事件，但正常发送通知；首行异常不影响整批通知 |

### 8.5 首行驱动 vs 逐行处理的容错差异

| 场景 | 首行驱动（分组/事件关联） | 逐行处理（去重） |
|------|--------------------------|-----------------|
| **首行字段缺失** | 回退到告警唯一键，整批使用同一指纹 | 只影响首行，其他行正常计算 |
| **首行指纹计算异常** | 回退到告警唯一键 | 只影响首行，抛出后被降级逻辑捕获 |
| **中间行字段异常** | 无感知（不处理中间行） | 该行被跳过或降级，其他行继续 |
| **数据库查询超时** | 整批失败，触发降级 | 该行失败，其他行继续；整体失败触发降级 |
| **并发冲突** | 整批使用同一指纹，冲突概率低 | 每行独立查询，冲突概率高，但单行失败不扩散 |
